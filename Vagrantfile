...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  # vm configuration
  config.vm.box = ENV['ANSIBLE_MAVEN_VAGRANT_BOXNAME'] || c['vm']['box']
  config.vm.box_check_update = c['vm']['check_update']

  # use insecure ssh for vagrant
  config.ssh.insert_key = false

  config.vm.define :ansiblemaventest do |d|

    d.vm.hostname = 'ansiblemaventest'
    ...
    d.vm.provision :ansible do |ansible|
      ...
      ansible.groups = {
        'vagrant' => ['ansiblemaventest']
      }
      ansible.limit = 'vagrant'

      #   dynamic ansible-playbook configuration based on environment variables
      ansible.tags = ENV['ANSIBLE_MAVEN_VAGRANT_ANSIBLE_TAGS']
      ansible.skip_tags = ENV['ANSIBLE_MAVEN_VAGRANT_ANSIBLE_SKIP_TAGS']
      #   dynamic ansible-playbook configuration based on environment variables or user configuration
      ansible.verbose = ENV['ANSIBLE_MAVEN_VAGRANT_ANSIBLE_VERBOSE'] || c['provisioner']['ansible']['verbose']

      # ansible-playbook raw arguments
      ansible.raw_arguments = []

      if ENV['ANSIBLE_MAVEN_VAGRANT_ANSIBLE_CHECKMODE'] == '1'
        ansible.raw_arguments << '--check'
      end

      if ENV['ANSIBLE_MAVEN_VAGRANT_ANSIBLE_DIFFMODE'] == '1' or c['provisioner']['ansible']['diff']
    ...
    d.vm.provider :virtualbox do |v|
      ...
      v.customize [ 'modifyvm', :id, '--name', 'ansiblemaventest', '--memory', '1024', '--cpus', '1' ]
    end
